...
raftLog* newLog(Storage *storage, Logger *logger) 
{
    ...
    if(storage ==  NULL)
    {
        logger->Panicf(__FILE__, __LINE__, "storage must not be nil");
    }
                          ...
                          uint64_t committed, const EntryVec& entries, uint64_t *lasti) 
{
    *lasti = 0;
    ...
    {
        logger_->Panicf(__FILE__, __LINE__, "entry %llu conflict with committed entry [committed(%llu)]", ci, committed_);
    }
...
uint64_t raftLog::append(const EntryVec& entries) 
{
    ...
    if (after < committed_) 
    {
        logger_->Panicf(__FILE__, __LINE__, "after(%llu) is out of range [committed(%llu)]", after, committed_);
    }
...
void raftLog::nextEntries(EntryVec* entries) 
{
    ...
    if (committed_ + 1 > offset) 
    {
        ...
        if (!SUCCESS(err)) 
        {
             logger_->Panicf(__FILE__, __LINE__, "unexpected error when getting unapplied entries (%s)", GetErrorString(err));
        }
    }
}
...
uint64_t raftLog::lastTerm() 
{
    ...
    if (!SUCCESS(err)) 
    {
        logger_->Panicf(__FILE__, __LINE__, "unexpected error when getting the last term (%s)", GetErrorString(err));
    }
...
int raftLog::mustCheckOutOfBounds(uint64_t lo, uint64_t hi) 
{
    ...
    if (hi > li + 1) 
    {
        logger_->Panicf(__FILE__, __LINE__, "slice[%llu,%llu) out of bound [%llu,%llu]", lo, hi, fi, li);
    }
...
uint64_t raftLog::zeroTermOnErrCompacted(uint64_t t, int err) 
{
    ...
    logger_->Panicf(__FILE__, __LINE__, "unexpected error: %s", GetErrorString(err));
    return 0;
}
